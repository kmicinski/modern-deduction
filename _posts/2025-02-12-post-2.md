...
This grounds the execution of `free` in something finite (our program, the input database). This fact that facts are first-class is the *only* change Slog makes from vanilla (textbook) Datalog. Formally, we explain Slog's formalization as unique existential quantification ([Section 2](https://arxiv.org/pdf/2411.14330v1)), such that it forms a more restricted version of existential Datalog (which is )
